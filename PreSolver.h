...
public:
    ...
    bool loadPreCalculated(string file)
    {
        ...
        if(!inContours.is_open())return false;*/
    ...
    void savePreCalculated(CGraph<Place,double>* oGraph, int k, string file)
    {
        ...
        /*
        //Calculamos los centroides de borde
        
        vector< vector<Node*> > contours;        
        vector<Node*> contour;
        
        for(int i=0;i<lsSegments.size();i++){
            contour = oSegment.getCentroids(lsSegments[i]);            
            contours.push_back(contour);
        }
                              
        ofstream fContours( sContours.append("_contours.txt") );
        
        for(int i=0;i<contours.size();i++){
            for(int j=0;j<contours[i].size();j++){
                fContours<<contours[i][j]<<" ";                
            }
            fContours<<endl;
        }        
        fContours.close();
