...
public:
    ...
    int calcularContorno(int idCluster)
    {
        ...
        for(int i=0; i<mGraph->nodes.size();i++)
        {
            ...
            {
                lsNodes.push_back(mGraph->nodes[i]);
            }
        }

        Segmentation oSegment;
        vector<CGraph<Place, double>::Node* >  lsContorno = oSegment.getCentroids(lsNodes);
        
        for(int i=0; i<lsContorno.size(); i++)
        {
            mDrawer.circFill(lsContorno[i]->data.x,lsContorno[i]->data.y,2,new Color(0.0,0.0,1.0,1.0));
        }
        
        return lsContorno.size();
    }
};